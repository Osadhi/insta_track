...
interface ListInterface{value:string, href:string}


const UploadPage: React.FC = () => {
    const [file1, setFile1] = useState<FileList | null>(null);
    const [file2, setFile2] = useState<FileList | null>(null);
    const [nonFollowers, setNonFollowers] = useState<ListInterface[]>([]);
    const [fans, setFans] = useState<ListInterface[]>([]);
    ...
    const handleCompare = async () => {
        try {
            ...
            const nonFollowersList: ListInterface[] = [];
            const fansList: ListInterface[] = [];

            following.relationships_following.forEach((entry1: ElementInterface) => {
                const matchingEntry = followers.find(
                    (entry2: ElementInterface) =>
            ...
            followers.forEach((entry1: ElementInterface) => {
